#!/bin/bash
# 云端服务器部署脚本 - 自动安装并配置 WireGuard

set -e

echo "=== WireGuard 内网穿透 - 云端部署 ==="
echo ""
...
# 安装依赖
echo "📦 安装依赖..."
yum install -y git golang wireguard-tools >/dev/null 2>&1 || apt install -y git golang wireguard-tools >/dev/null 2>&1
